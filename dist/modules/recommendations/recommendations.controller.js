...
const problem_model_1 = require("../dsa/problem.model");
...
exports.recommendationsController = {
    async getRecommendations(req, res) {
        try {
            const problems = await problem_model_1.Problem.find({ userId: req.user._id, status: 'Solved' });
            // Check if filtering by specific topic
            const filterTopic = req.query.topic;
            ...
            for (const [topic, target] of Object.entries(TOPIC_TARGETS)) {
                ...
                const suggestions = allSuggestions.filter(s => !solvedTitles.some(t => t.includes(s.toLowerCase()))).slice(0, maxSuggestions);
