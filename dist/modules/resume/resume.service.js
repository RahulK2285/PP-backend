...
async function extractTextFromPDF(filePath) {
    try {
        ...
        const pdfParse = typeof require !== 'undefined' ? require('pdf-parse') : (await Promise.resolve().then(() => __importStar(require('pdf-parse')))).default;
...
function scoreResume(text, mimeType) {
    ...
    if (hasEmail)
        ...
        tips.push('Add your email address for recruiters to contact you');
    if (hasPhone)
        contactScore += 3;
    else
        tips.push('Include a phone number for direct communication');
    if (hasLinkedin)
        contactScore += 3;
    else
        tips.push('Add your LinkedIn profile link');
    if (hasGithub)
        contactScore += 3;
    else
        tips.push('Include your GitHub profile to showcase your code');
    if (hasPortfolio)
        contactScore += 2;
    categories.push({
    ...
    if (hasSummary) {
        ...
        if (summaryIdx !== -1) {
            ...
            if (afterSummary.length > 100)
                summaryScore += 3;
            if (afterSummary.length > 200)
                summaryScore += 2;
        }
    }
    else {
    ...
    if (hasEducation) {
        educationScore += 4;
        if (/\d{4}/.test(text))
            educationScore += 2; // Has graduation year
        if (/cgpa|gpa|percentage|%/.test(lowerText))
            educationScore += 2; // Has GPA
        if (/b\.?tech|m\.?tech|bachelor|master|b\.?e|b\.?sc|m\.?sc/.test(lowerText))
            educationScore += 2; // Has degree name
    }
    else {
    ...
    if (hasExperience) {
        ...
        if (hasNumbers)
            ...
            tips.push('Add quantifiable achievements (e.g., "Increased performance by 40%")');
    }
    else {
    ...
    if (hasSkills)
        skillsScore += 3;
    if (techKeywordsFound.length >= 10)
        skillsScore += 6;
    else if (techKeywordsFound.length >= 6)
        skillsScore += 5;
    else if (techKeywordsFound.length >= 3)
        skillsScore += 3;
    else if (techKeywordsFound.length >= 1)
        skillsScore += 2;
    // Bonus for organized skills section
    if (techKeywordsFound.length >= 5 && hasSkills)
        skillsScore += 3;
    ...
    if (hasLanguage && hasFramework && hasDatabase)
        skillsScore += 3;
    ...
    if (hasProjects) {
        ...
        if (hasLinks)
            ...
            tips.push('Add live demo or GitHub links for your projects');
    ...
    if (wordCount >= 200 && wordCount <= 1200) {
        ...
        formatScore += 2;
        if (wordCount > 1200)
            tips.push('Your resume may be too long — try to keep it to 1-2 pages');
        else
            tips.push('Your resume seems short — add more details about your experience and projects');
    }
    else {
        if (isTextExtracted)
            tips.push('Resume content is very thin — aim for at least 1 full page');
    }
    ...
    if (hasBullets)
        ...
        tips.push('Use bullet points to organize your experience and achievements');
    ...
    if (hasCerts) {
        ...
        tips.push('Add certifications (AWS, Google, Coursera, etc.) to boost credibility');
    }
    if (hasAchievements) {
        extrasScore += 4;
    }
    ...
    if (!isTextExtracted && mimeType !== 'application/pdf') {
        ...
        return {
            ...
            percentage: 0,
            grade: 'N/A',
            categories: categories.map(c => ({ ...c, score: 0, feedback: 'Upload PDF for analysis', status: 'warning' })),
            tips,
        };
    }
    // ── Calculate Total ──
